...
def crear_data_experimento(instancia, dim, mh, binarizacion, iteraciones, poblacion, extra_params, problemaActual):
    ...
    if problemaActual == 'BEN':
        print(extra_params)
        if "chaotic_map" in extra_params:
            nombre_experimento = f'{instance_tuple[1]} {dim} {extra_params.split(",")[3].split(":")[1]}'
        else:
            nombre_experimento = f'{instance_tuple[1]} {dim}'
    else: # SCP, USCP, KP
...
def agregar_experimentos():
    if config.get('ben', False):
        ...
        if config['experimentos']['BEN']['chaotic_map'] == "":
            extra_params = f",cross:{config['experimentos']['BEN']['cross']},muta:{config['experimentos']['BEN']['muta']}" # Parámetros por defecto para GA en BEN
        else:
            extra_params = f",cross:{config['experimentos']['BEN']['cross']},muta:{config['experimentos']['BEN']['muta']},chaotic_map:{config['experimentos']['BEN']['chaotic_map']}" # Parámetros por defecto para GA en BEN
        ...
        for funcion in config['instancias']['BEN']:
            ...
            insertar_experimentos(instancias_ben, dimensiones_ben, config['mhs'], num_experimentos, iteraciones, poblacion, problemaActual='BEN', extra_params=extra_params)
        ...
        if activar:
            ...
            for instancia_db_tupla in instancias_db: # instancia_db_tupla es (id_instancia, nombre_instancia, ...)
                ...
                if problema in ['SCP', 'USCP']:
                    extra_params = f",repair:{config['experimentos'][problema]['repair']},cross:{config['experimentos'][problema]['cross']},muta:{config['experimentos'][problema]['muta']}"
                elif problema == 'KP':
                    ...
                    # y leerlo.
                    # Ejemplo:
                    extra_params = f",cross:{config['experimentos'][problema]['cross']},muta:{config['experimentos'][problema]['muta']}"
