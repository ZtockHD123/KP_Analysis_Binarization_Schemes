...
def obtenerExperimentosCSV():
    ruta_fitness = './Resultados/Test/SCP/data/'
    df = pd.read_csv(os.path.join(ruta_fitness, '41.csv'))
...
def test_estadistico():
    ...
    binarizacion_en_analisis = ['ELIT', 'ELIT_TENT', 'ELIT_SINU', 'ELIT_LOG']
    if generar_datos:
        ruta_base = './Resultados/data/SCP/fitness/'
        ...
        for instancia in instancias:
            ...
            df_unificado.to_csv(f'./Resultados/Test/SCP/data/{instancia[1]}.csv', index=False)
    ...
    if test_wilconxon:
        ...
        ruta_fitness = './Resultados/Test/SCP/data/'
        ruta_salida_base = './Resultados/Test/SCP/wilconxon/'
    ...
    if test_por_algoritmo:
        ...
        ruta_wilcoxon = './Resultados/Test/SCP/wilconxon/'
        ruta_salida_base = './Resultados/Test/SCP/test_per_algoritmo/'
    ...
    if tabla_doble_entrada:
        ...
        ruta_per_algoritmo = './Resultados/Test/SCP/test_per_algoritmo/'
        ruta_salida = './Resultados/Test/SCP/'
...
def graficar_exploracion_explotacion(iteraciones, xpl, xpt, titulo, mh, binarizacion):
    ...
    output_dir = './Resultados/xpl_xpt/SCP/best/'
...
def graficar_dos_series_con_zoom(eje_x, 
                                 ...
                                 zoom_ylim=(0, 5000)):   # Rango Y para el zoom (ajusta según tus datos)
    """
    ...
    for instancia in instancias:
        ...
        for mh in mhs:
            ...
            graficar_dos_series_con_zoom(
                ...
                etiqueta_y="Fitness",
                ruta_guardado=f"./Resultados/best/SCP/convergencia_{titulo}_{mh[0]}.png"
            )
        ...
        for instancia in instancias:
            ...
            for mh in mhs:
                for binarizacion in binarizacion_en_analisis:
                    ...
                    df_xpt.to_csv(f'./Resultados/xpl_xpt/SCP/data/{titulo}_{mh[0]}_{binarizacion}_xpt.csv', index=False)
                    df_xpl.to_csv(f'./Resultados/xpl_xpt/SCP/data/{titulo}_{mh[0]}_{binarizacion}_xpl.csv', index=False)
    
    if bandas_xpl_xpt:            
        ruta_base_data = './Resultados/xpl_xpt/SCP/data/'
        for instancia in instancias:
            for mh in mhs:
                for binarizacion in binarizacion_en_analisis:
                    ...
                    ruta_salida = f'./Resultados/xpl_xpt/SCP/bandas/{instancia[1]}_{mh[0]}_{binarizacion}_comparativo.png'
                    graficar_bandas_xpl_xpt(ruta_xpl, ruta_xpt, ruta_salida)

def generar_analitica_familiar():
    configuraciones = ['ELIT', 'ELIT_LOG', 'ELIT_PIECE', 'ELIT_SINE', 'ELIT_SINGER', 'ELIT_SINU', 'ELIT_TENT', 'ELIT_CIRCLE']
    ...
    columnas =  ['Instance','Opt', 'ELIT', '', 'ELIT_LOG', '', 'ELIT_PIECE', '', 'ELIT_SINE', '', 'ELIT_SINGER', '', 'ELIT_SINU', '', 'ELIT_TENT', '', 'ELIT_CIRCLE', '']
    encabezado =  ['', '','best', 'avg', 'best', 'avg', 'best', 'avg', 'best', 'avg', 'best', 'avg', 'best', 'avg', 'best', 'avg','best', 'avg']
    mhs = bd.obtenerMHBD()
    for mh in mhs:
        # 1. Creas tu buffer
        filas_acumuladas = [encabezado]
        archivos = os.listdir(f'./Resultados/data/SCP/fitness/{mh[0]}/')
        for archivo in archivos:
            fila_dinamica = [archivo.split(".")[0]]  # La primera columna es el nombre de la instancia
            df = pd.read_csv(f'./Resultados/data/SCP/fitness/{mh[0]}/{archivo}')
        ...
        df_final.to_csv(f'./Resultados/resumen/SCP/fitness/{mh[0]}.csv', index=False)
        print(f"✅ Archivo fitness_SCP_{mh[0]}.csv generado exitosamente.")
...
def generarRPD():
    ...
    for mh in mhs:
        ...
        df_final.to_csv(f'./Resultados/resumen/SCP/RPD/{mh[0]}.csv', index=False)
        print(f"✅ Archivo RPD_SCP_{mh[0]}.csv generado exitosamente.")
...
def generar_data():
    instancias = bd.obtenerInstanciasEjecutadas("SCP")
    print(f"[INFO] Analizando {len(instancias)} instancias de SCP...")
    ...
    for instancia in instancias:
        ...
        for mh in mhs:
            ...
            for binarizacion in binarizaciones:
                ...
                ruta_fitness = f'./Resultados/data/SCP/fitness/{mh[0]}/{instancia[1]}.csv'
                ruta_tiempos = f'./Resultados/data/SCP/tiempos/{mh[0]}/{instancia[1]}.csv'
...
def analizar_instancias():
    ...
    if GENERAR_GRAFICOS_DISTRIBUCION:
        ...
        for instancia in instancias:
            mhs = bd.obtenerMHEjecutadas(instancia[1])
            for mh in mhs:
                ruta_entrada = f'./Resultados/data/SCP/fitness/{mh[0]}/{instancia[1]}.csv'
                ruta_salida_box = f'./Resultados/boxplot/SCP/{mh[0]}/'
